...
FONT_GENEVA_9 = ImageFont.truetype(os.path.join(FONT_DIR, 'wqy_dianzhen_zhenghei.ttf'), 12)
...
def draw_list_window(ink_draw: ImageDraw, ink_image: Image, origin: Tuple[int, ...], event_list: List[Event]):
    ...
    for event in event_list:
        ...
        ink_image.paste(folder, (origin[0] + 6, v_offset + 3)) # If you use wqy series fonts, you should uncomment this line.
...
def draw_scrapbook(ink_draw: ImageDraw, ink_image: Image, origin: Tuple[int, ...], date_str: str, adornments: Tuple[str, ...], forecast: list, enabled: bool):
    ...
    if date_str is not None:
        (text_wide, text_high) = ink_draw.multiline_textsize(date_str, font=FONT_GENEVA_9)
        ink_draw.text((origin[0] + 191 - int(text_wide / 2), origin[1] + 42), date_str, font=FONT_GENEVA_9, fill=1)
        ink_draw.text((origin[0] + 192 - int(text_wide / 2), origin[1] + 42), date_str, font=FONT_GENEVA_9, fill=1)
